...
\subsection{Wurzelortskurven (WOK)-Verfahren}
	...
	\begin{align*}
		...
		& =K \cdot Q \cdot \frac{\prod_{\mu=1}^{m}\left(s-s_{N0\mu}\right)}{\prod_{\theta=1}^{n}\left(s-s_{P0\theta}\right)}
	\end{align*}

	Für eine Polstele, muss der Nenner von $F_w (s)$ Null werden:
	\begin{align*}
		\dfrac{F_R (s) \cdot F_S (s)}{1+F_o (s)} \Rightarrow 1+F_o (s) \stackrel{!}{=}0 \\
		\Rightarrow \frac{\prod_{\theta=1}^{n}\left(s-s_{P0\theta}\right)}{\prod_{\mu=1}^{m}\left(s-s_{N0\mu}\right)} = -K \cdot Q
	\end{align*}

	Diese Gleichung kann in Betrag und Phase aufgeteilt werden:
	\begin{align*}
		& \frac{\prod_{\theta=1}^{n}\left|s-s_{P0\theta}\right|}{\prod_{\mu=1}^{m}\left|s-s_{N0\mu}\right|} = |K \cdot Q| \\
		& \sum_{\theta=1}^{n} \angle\left(s-s_{P0\theta}\right) - \sum_{\mu=1}^{m} \angle\left(s-s_{N0\mu}\right) = \angle\left(-K \cdot Q\right) = \pm r \cdot 180^\circ \\
